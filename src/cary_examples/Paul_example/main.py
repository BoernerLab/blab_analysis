...
class MeltingCurveAnalysis:
    ...
    def _get_filtered_data(self, measurement_number):
        filtered_data = self.current_data.data[self.current_data.data['Measurement']
                                               == measurement_number].copy().sort_values('Temperature (°C)')
        filtered_data['Temperature (K)'] = filtered_data['Temperature (°C)'] + 273.15 
 ...
 #   @staticmethod
    ...
    def normalize_data(df: pd.DataFrame,
        ...
        """
        Normalize melting curve dataframe to a given temperature values or to min and max value. The normalized values are added to the dataframe.
    ...
    def weigh_by_values(df: pd.DataFrame, weigh_bounds: dict = {'column_x': 'Temperature (°C)', 'column_y': 'Absorbance', 'x_min': -np.inf, 'y_min':
                             - np.inf, 'x_max': np.inf, 'y_max': np.inf, 'factor':1}):
        """
        Crop/remove parts of a curve with x and y values.-
        ...
        """
        weigh_bounds_updated = {'column_x': 'Temperature (°C)', 'column_y': 'Absorbance', 'x_min': -np.inf, 'y_min':
                                 - np.inf, 'x_max': np.inf, 'y_max': np.inf, 'factor': 1}
        weigh_bounds_updated.update(weigh_bounds) 
        column_x = weigh_bounds_updated.get('column_x')
        column_y = weigh_bounds_updated.get('column_y')
        df_filtered = df.copy() 
        df_filtered['weigh'] = 1
        df_filtered.loc[
            (df_filtered[column_x] >= weigh_bounds_updated['x_min']) & (df_filtered[column_x] <= weigh_bounds_updated['x_max']) &
            (df_filtered[column_y] >= weigh_bounds_updated['y_min']) & (
                df_filtered[column_y] <= weigh_bounds_updated['y_max']), :] = weigh_bounds_updated.get('factor')
        return df_filtered
        
    def weigh_multiple_times(df: pd.DataFrame, weigh_bounds_list: list = []):
        """
        Filter a curve multiple times.
        ...
        """
        if not weigh_bounds_list:
            return df
        filtered_df = df.copy()
        temp_list = np.zeros_like(weigh_bounds_list)
        for index, value in enumerate(weigh_bounds_list): 
            filtered_df= MeltingCurveAnalysis.filter_by_values(filtered_df, value)
    ...
    def remove_measurements(self, remove_list: list = []):
        """
        Remove specific curves from your data. 

        Parameters
        ----------
        remove_list : list, optional
            List of lists. Each list contanins the filename and its measurement index as values (2 elements). The default is [].
    ...
    def equation_molecularity_1_dCp(T, DH, DCp, Tm, m1, n1, m2, n2):
        """
        Analytical melting curve function for intramolecular reactions. Böttcher et. al., ...
        ...
        """
        R = 8.31446261815324  # m^3* Pa * K^−1 * mol^−1
        x = DH / R * (1 / T - 1 / Tm ) - DCp/R * np.log(T/Tm) + DCp/R * (1 - Tm/T)
        theta = 1 / (1 + np.exp(x)) 
        return (m1 * T + n1) * theta + (m2 * T + n2) * (1 - theta)
    ...
    def equation_molecularity_2_dCp(T, DH, DCp, Tm, m1, n1, m2, n2):
        """
        Analytical melting curve function for intramolecular reactions. Böttcher et. al., ...
    ...
    def create_model_e1():
        ...
        """
        e1_mod = lmfit.Model(MeltingCurveAnalysis.equation_molecularity_1)
    ...
    def create_model_e2():
        ...
        """
        e2_mod = lmfit.Model(MeltingCurveAnalysis.equation_molecularity_2)
    ...
    def create_model_e2_e1():
        ...
        pars = e2_mod.make_params(DH=dict(value=-300000, max=0), Tm=dict(value=325,
                                  min=250, max=375), m1=0.0005, m2=0.0005, n1=0.1, n2=0.2)
        pars.update(e1_mod.make_params(DH=dict(value=-300000, max=0), Tm=dict(value=345, min=250, max=375),
                                       m1=dict(expr='e2_m2'), m2=0.0005, n1=dict(expr='e2_n2'), n2=0.3))
    ...
    def direct_melt_fit(self, filename, measurement_number,
        ...
        df_data_raw['Temperature (K)'] = df_data_raw.loc[:, 'Temperature (°C)'] + 273.15 
    ...
    def linear_baseline_fit(self,
        ...
        df_data_raw, second_derivative_name = MeltingCurveAnalysis._calculate_derivative(
            df_data_raw.sort_values(column_x),
            column_x=column_x,
            column_y=first_derivative_name,
            first_derivative_name=second_derivative_name)
        ...
        if maxima == None:
            ...
            lower_baseline = df.loc[((abs(df[first_derivative_name]) < deribounds1[0]) & (abs(df[second_derivative_name]) < deribounds2[0])
                                     & (df[column_x] < maxima[f'{column_x}_max'])), :]
            # & (df['Temperature (C)'] >= upper_baseline_bounds[0]) & (df['Temperature (C)'] <= upper_baseline_bounds[1])), :]
            upper_baseline = df.loc[((abs(df[first_derivative_name]) < deribounds1[1]) & (abs(df[second_derivative_name]) < deribounds2[1])
                                     & (df[column_x] > maxima[f'{column_x}_max'])), :]
      ...
      #      print(delta_Ed, '\n')
        ...
        #    print("Closest intersection point with higher x accuracy:", closest_intersection)
         #   med_data['Intersection'] = closest_intersection
            ...
            self.data[filename][f'Measurement_{measurement_number}']['data'] = df_data_raw
   ...
   #     print(max_peaks_pl)
    ...
    def _folded_fraction_formula(y: list, baseline_lower: list, baseline_upper: list):
        """
        Calculate folded fraction of a 2 state transission.

        Parameters
        ----------
        y : list
            Melting Curve Values, for example Absorbance values.
        baseline_lower : list
            Values of the lower baseline.
        baseline_upper : list
            Values of the upper baseline.

        Returns
        -------
        TYPE
            Folded fraction values.
...
if __name__ == "__main__":
    ...
    test.meta_from_name_Paul(
        ['2023-11-17_RNA_melting_MgCl2_NaCl.csv', '2023_08_09_DNA_MOPS_75_K.csv'])
